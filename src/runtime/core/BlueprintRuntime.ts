...
import type { BlueprintClassParseRuntime, BlueprintNodeConstructor } from "./BlueprintFactory";
...
export class BlueprintRuntime implements BlueprintClassParseRuntime {
  ...
  public readonly funBlockMap: Map<string, BluePrintFunBlock>;
  public isRunningInIDE: boolean;
  public readonly mainBlock: BluePrintMainBlock;
  constructor() {
  ...
  public runCustomFun(
    ...
    const fun = funId ? this.funBlockMap.get(funId) : undefined;
    if (fun) {
      return fun.run(
        context,
        null,
  ...
  public parse(
    ...
    const bpjson = mainBlockData.arr;
  ...
  public parseFunction(
    ...
    let _a;
    const funId = String(funData.id),
      bpjson = funData.arr as BlueprintJsonItem[];
    const fun = new BluePrintFunBlock(funId);
    fun.isStatic =
    ...
    const varMap: BlueprintVariableMap = {};
    const variables = funData.variable as Array<{ name: string }> | undefined;
    if (variables) {
      variables.forEach((item) => {
        varMap[item.name] = { value: undefined, ...item };
      });
    }
  ...
  public toCode(_context: BlueprintExecuteNode) {}
}
