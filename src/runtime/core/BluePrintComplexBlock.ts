...
import type { BlueprintDataMap, BlueprintJsonItem } from "./BlueprintJsonTypes";
...
export class BluePrintComplexBlock extends BluePrintBlock {
    ...
    public parse(
        ...
        bpjson.forEach((item) => {
            let node = BlueprintFactory.instance.createNew(
        ...
        bpjson.forEach((item) => {
            this.getNodeById(item.id as string | number).parseLinkData(item, this);
        });
        this.onParse(bpjson);
        this.optimize();
    }
    ...
    private _checkReady(
        ...
        bpjson.forEach((item) => {
            ...
            if (!itemdef) {
                let classID = item.target as string;
                if (!classID) {
                ...
                if (def.type == BPType.Event && item.output && item.output["out_" + EXECID]) {
