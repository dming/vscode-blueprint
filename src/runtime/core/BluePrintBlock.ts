...
export class BluePrintBlock {
  ...
  public onParse(_bpjson: BlueprintJsonItem[]) {}
  ...
  public runAnonymous(
    ...
    const anonymousBlock = this.anonymousBlockMap.get(String(event.nid))!;
    if (anonymousBlock.haRef) oldRuntimeDataMgr.saveContextData(runId, newRunId);
    return anonymousBlock.run(context, event as BlueprintEventNode, parms, cb, newRunId, execId);
  }
    ...
    for (let i = currentIndex, n = executeAbleList.length; i < n; ) {
      ...
      if (index instanceof BlueprintPinRuntime) {
        ...
        if (fromPin == null) {
          ...
          const nextI = idToIndex.get(fromPin.owner.nid);
          if (nextI === undefined) {
            break;
          }
          i = nextI;
  ...
  public finish(_context: BlueprintExecuteNode) {}
}
