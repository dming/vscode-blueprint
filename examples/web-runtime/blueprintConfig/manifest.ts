 ...
 */
import { SAMPLE_EDITOR_HOST_BASE_CLASS } from "./constants";
...
class _SampleActorHost {
  @Lifecycle() onBeginPlay() {}
  @Lifecycle({ outputs: ["deltaTime:number"] }) onTick() {}
  @Lifecycle() onEndPlay() {}
}

@BlueprintBaseClass(SAMPLE_EDITOR_HOST_BASE_CLASS)
class _SampleComponentHost {
  @Lifecycle() onAwake() {}
  @Lifecycle() onStart() {}
  @Lifecycle({ outputs: ["deltaTime:number"] }) onUpdate() {}
  @Lifecycle() onDestroy() {}
}

void _SampleActorHost;
void _SampleComponentHost;
...
class _SampleNodePalette {
  ...
  @BlueprintNodeDef({
    ...
    outputs: ["exec:exec"],
    defaults: { text: "Hello from Blueprint sample" },
